...
exports.cardMentioned = function() {
  ...
  client.addListener('message', function(from, to, text, message) {
    ...
    if(self.findCardMention(messageString)) {
      ...
      kanbanery.get(cardID, 'tasks', function(err, response) {
        if (err || !response) {
          console.log('Error fetching task info.. awww:' + err);
        } else {
          client.say(channel, response.title + " :: Column: " + "column" + ":: Blocked:" + response.blocked);
        }
      });          
    }    
  });  
